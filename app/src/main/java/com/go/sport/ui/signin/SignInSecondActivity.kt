...
import kotlinx.android.synthetic.main.activity_sign_in_second.*
import retrofit2.HttpException
import java.io.IOException
import java.util.concurrent.TimeUnit

class SignInSecondActivity : BaseActivity() {
...
/*        fullScreen()
        ...
        if (BuildConfig.DEBUG) {
            et_phone_number.setText("893124950")
    ...
    private fun setFonts() {
        setFont(Constants.MEDIUM, til_phone_number)
        setFont(Constants.MEDIUM, til_password)
    }
    ...
    private fun initListeners() {
       // ccp.text = "+372"
        ...
        /*     APIManager.getCountry { result, error ->
                 if (result == "PK") {
                     ccp.text = "+92"
                     countryCode = "+92"
                 } else if (result == "AE") {
     ...
     */
        ...
        RxView.clicks(cont_create_account).throttleFirst(2, TimeUnit.SECONDS).subscribe {
...
//            LoginManager.getInstance().logInWithReadPermissions(this, listOf("public_profile"))
    ...
    private fun loginUser() {
        ...
        pBar(1)
        APIManager.login(countryCode + phoneNumber, password) { result, error ->
            pBar(0)
